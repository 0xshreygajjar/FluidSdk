...
import type { AgentSummary, SearchParams, SearchResultMeta, AgentSearchResult, AgentSearchResultWithMeta, ChainQueryResult, AgentWithScore } from '../types/interfaces.js';
import type { AgentId, ChainId } from '../types/common.js';
...
export class AgentIndexer {
  ...
  private _sortAgentsCrossChain(agents: AgentSummary[], sort: string[]): AgentSummary[] {
    ...
    const sortField = firstSort.split(':');
  ...
  private async _searchAgentsAcrossChains(
  ...
  ): Promise<AgentSearchResultWithMeta> {
    ...
    for (let i = 0; i < chainResults.length; i++) {
      ...
      // Skip if chainId is undefined (shouldn't happen, but for type safety)
  ...
  private async _searchAgentsByReputationAcrossChains(
    ...
    const querySingleChain = async (chainId: ChainId): Promise<ChainQueryResult> => {
      try {
        const subgraphClient = this._getSubgraphClientForChain(chainId);
        ...
        try {
          ...
          // Transform to AgentSummary
          const transformedAgents: AgentSummary[] = agentsData.map((agent) => {
            const regFile = agent.registrationFile;
            return {
              chainId: parseInt(agent.chainId?.toString() || '0', 10),
              agentId: agent.id || '',
              ...
              x402support: regFile?.x402support ?? false,
              extras: {
                averageScore: agent.averageScore !== null && agent.averageScore !== undefined ? agent.averageScore : undefined,
              },
            };
          });
    ...
    // Agents are already transformed to AgentSummary in querySingleChain
